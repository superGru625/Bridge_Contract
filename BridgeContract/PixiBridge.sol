...
contract PixiBridge is Ownable {
    ...
    */
    ...
    function setPixiContract(address _PixiContract) external onlyOwner returns(bool) {
        PixiContract = _PixiContract;
        return true;
    }
    ...
    external payable notPaused returns (bool) {
        require(msg.sender == PixiContract, "Only Pixi");
    ...
    external onlySystem notPaused returns (bool) {
        ...
        if(path[path.length-1] == USDT) {
            ...
            IGatewayVault(gatewayVault).vaultTransfer(USDT, PixiContract, amount); 
            IPixi(PixiContract).callbackCrossExchange(orderType, path, amount, user, dexId, deadline);
        }
    ...
    // `destReceiver` - Pixi contract address
        ...
        notPaused returns (bool) 
    {
        ...
        //IPixi(PixiContract).callbackCrossExchange1Inch(USDT, amount, to, data);
